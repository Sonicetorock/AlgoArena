...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    const totalAcceptedSubmissions = await Problem.aggregate([
      { $group: { _id: null, total: { $sum: "$acceptedSols" } } },
    ]);
    const totalRejectedSubmissions = await Problem.aggregate([
      {
        $group: {
          _id: null,
          total: {
            $sum: { $subtract: ["$totalSubmissions", "$acceptedSols"] },
          },
        },
      },
    ]);
...
exports.getProblem = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Problem recieved successfully", problem });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
...
exports.getAllUsers = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || "error fetching all users" });
  }
};
  ...
  try {
    ...
    for (let i = 11; i >= 0; i--) {
      ...
      userCountsArray.push(
        userCounts.countsByMonth.filter((item) => item.date <= monthEnd).length
      );
      submissionCountsArray.push(
        submissionCounts.countsByMonth.filter((item) => item.date <= monthEnd)
          .length
      );
      questionCountsArray.push(
        questionCounts.countsByMonth.filter((item) => item.date <= monthEnd)
          .length
      );
    }
